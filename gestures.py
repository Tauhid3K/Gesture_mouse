...
def process_scroll_motion(mid_x, mid_y, scroll_active, prev_scroll_x, prev_scroll_y, filtered_scroll_offset_x, filtered_scroll_offset_y, current_scroll_multiplier):
    """Apply shared vertical/horizontal scroll logic and return updated scroll state.
    ...
    if not scroll_active:
        ...
        scroll_active = True
        prev_scroll_x = mid_x      # Fixed anchor X
        prev_scroll_y = mid_y      # Fixed anchor Y (will NOT be updated while scroll_active)
        filtered_scroll_offset_x = 0.0
        filtered_scroll_offset_y = 0.0
        if DEBUG_SCROLL:
            print(f"[SCROLL:START] anchor_y={mid_y:.4f}, multiplier={current_scroll_multiplier}")
    else:
        # Calculate offset from FIXED anchor point (prev_scroll_y stays constant while scrolling)
        raw_offset_y = mid_y - prev_scroll_y
        filtered_scroll_offset_y += (raw_offset_y - filtered_scroll_offset_y) * SCROLL_SMOOTH_ALPHA
        ...
        scroll_offset_y = filtered_scroll_offset_y
        ...
        scroll_amount = int(round(base_scroll_amount * current_scroll_multiplier))
        if scroll_amount != 0:
            pyautogui.scroll(scroll_amount)
            if DEBUG_SCROLL:
                print(f"[SCROLL:V] raw_offset={raw_offset_y:.5f} filt={filtered_scroll_offset_y:.5f} base={base_scroll_amount} final={scroll_amount} multiplier={current_scroll_multiplier}")

        # Optional horizontal mode while scrolling: drift hand left/right from anchor.
        raw_offset_x = mid_x - prev_scroll_x
        filtered_scroll_offset_x += (raw_offset_x - filtered_scroll_offset_x) * SCROLL_SMOOTH_ALPHA
        hscroll_amount = offset_to_scroll_step(filtered_scroll_offset_x, SCROLL_HORIZONTAL_GAIN)
        if hscroll_amount != 0:
            apply_horizontal_scroll(hscroll_amount)
            if DEBUG_SCROLL:
                print(f"[SCROLL:H] raw_offset={raw_offset_x:.5f} amount={hscroll_amount}")

        # IMPORTANT: Do NOT update prev_scroll_x and prev_scroll_y!
...
def process_right_hand_gestures(right_lm, virtual_left, virtual_top, screen_w, screen_h, cursor_x, cursor_y, current_dpi, current_speed_x, current_speed_y, current_deadzone, current_smooth, current_scroll_multiplier, prev_mid_x, prev_mid_y, filtered_dx, filtered_dy, pinch_start_time, is_dragging, last_left_click, scroll_active, prev_scroll_x, prev_scroll_y, right_touch_prev, last_right_click, last_fist_click, last_three_finger_click, fist_prev, now, filtered_scroll_offset_x, filtered_scroll_offset_y):
    ...
    if prev_mid_x is None:
        prev_mid_x, prev_mid_y = mid_x, mid_y
    else:
        raw_dx = mid_x - prev_mid_x
        raw_dy = mid_y - prev_mid_y
        prev_mid_x, prev_mid_y = mid_x, mid_y

        if abs(raw_dx) < current_deadzone:
            raw_dx = 0.0
        if abs(raw_dy) < current_deadzone:
            raw_dy = 0.0

        filtered_dx += (raw_dx - filtered_dx) * current_smooth
        filtered_dy += (raw_dy - filtered_dy) * current_smooth

        if not scroll_gesture:
            cursor_x += filtered_dx * screen_w * current_speed_x * current_dpi
            cursor_y += filtered_dy * screen_h * current_speed_y * current_dpi
            cursor_x = max(float(virtual_left), min(float(virtual_left + screen_w - 1), cursor_x))
            cursor_y = max(float(virtual_top), min(float(virtual_top + screen_h - 1), cursor_y))
            pyautogui.moveTo(cursor_x, cursor_y)

    # Left click on fist close.
    fist_now = is_fist_gesture(right_lm)
    if fist_now and not fist_prev:
        if (now - last_fist_click) > 0.3:
    ...
    if left_touch:
        ...
        elif not is_dragging and (now - pinch_start_time) >= 0.5:
        ...
        if pinch_start_time is not None:
            ...
            if is_dragging:
                ...
                is_dragging = False
            elif pinch_duration < 0.5 and (now - last_left_click) > 0.12:
                send_mouse_click(button="left")
                last_left_click = now
    ...
    if three_finger_touch and (now - last_three_finger_click) > 0.3:
        send_mouse_click(button="left")
        last_three_finger_click = now
    ...
    if right_touch and not right_touch_prev and (now - last_right_click) > 0.5:
        send_mouse_click(button="right")
        last_right_click = now
    right_touch_prev = right_touch
